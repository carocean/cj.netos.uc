...
public interface IAppRolePort extends IOpenportService {
    ...
    @CjOpenport(usage = "移除角色",elementType = AppRole.class)
    List<AppRole> pageRole(@CjOpenportParameter(usage = "当前页", name = "currPage") int currPage, @CjOpenportParameter(usage = "页大小", name = "pageSize") int pageSize) throws CircuitException;

    @CjOpenport(usage = "移除角色",elementType = AppRole.class)
    List<AppRole> pageUserInRole(@CjOpenportParameter(usage = "角色标识", name = "roleid") String roleid, int currPage, int pageSize) throws CircuitException;
