...
public class TenantAccountService implements ITenantAccountService, IServiceSetter {
    ...
    public String addAccount(TenantAccount account) throws CircuitException {
        if (existsAccount(account.getTenantId(), account.getUserId(), account.getAccountName())) {
    ...
    @CjTransaction
    @Override
    public boolean existsAccount(String tenant, String userId, String accountName) throws CircuitException {
        TenantAccountExample example = new TenantAccountExample();
        example.createCriteria().andTenantIdEqualTo(tenant).andUserIdEqualTo(userId).andAccountNameEqualTo(accountName);
