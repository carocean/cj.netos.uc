package cj.netos.uc.plugin.dao;

import cj.netos.uc.domain.UaAppRoleUserExample;
import cj.netos.uc.domain.UaAppRoleUserKey;
import java.util.List;
import org.apache.ibatis.annotations.Param;

public interface UaAppRoleUserMapper {
    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table ua_app_role_user
     *
     * @mbg.generated Mon Sep 16 16:08:22 PHT 2019
     */
    long countByExample(UaAppRoleUserExample example);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table ua_app_role_user
     *
     * @mbg.generated Mon Sep 16 16:08:22 PHT 2019
     */
    int deleteByExample(UaAppRoleUserExample example);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table ua_app_role_user
     *
     * @mbg.generated Mon Sep 16 16:08:22 PHT 2019
     */
    int deleteByPrimaryKey(UaAppRoleUserKey key);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table ua_app_role_user
     *
     * @mbg.generated Mon Sep 16 16:08:22 PHT 2019
     */
    int insert(UaAppRoleUserKey record);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table ua_app_role_user
     *
     * @mbg.generated Mon Sep 16 16:08:22 PHT 2019
     */
    int insertSelective(UaAppRoleUserKey record);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table ua_app_role_user
     *
     * @mbg.generated Mon Sep 16 16:08:22 PHT 2019
     */
    List<UaAppRoleUserKey> selectByExample(UaAppRoleUserExample example);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table ua_app_role_user
     *
     * @mbg.generated Mon Sep 16 16:08:22 PHT 2019
     */
    int updateByExampleSelective(@Param("record") UaAppRoleUserKey record, @Param("example") UaAppRoleUserExample example);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table ua_app_role_user
     *
     * @mbg.generated Mon Sep 16 16:08:22 PHT 2019
     */
    int updateByExample(@Param("record") UaAppRoleUserKey record, @Param("example") UaAppRoleUserExample example);
}